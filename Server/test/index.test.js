...
describe("Test de RUTAS", () => {
  ...
  describe("GET /rickandmorty/login", () => {
    it("Ruta de login retorna access true", async () => {
      const response = (
        await agent.get(
          "/rickandmorty/login?email=ejemplo@gmail.com&password=rodrigo1"
        )
      ).body;
    ...
    it("Ruta de login retorna access false", async () => {
      const response = (
        await agent.get(
          "/rickandmorty/login?email=mplo@gmail.com&password=rodrigo"
        )
      ).body;
  ...
  describe("POST /rickandmorty/fav", () => {
    ...
    it("Devuelve el arreglo enviado por body con 2 personajes", async () => {
  ...
  describe("DELETE /rickandmorty/fav/:id", () => {
    const character1 = { id: "1", name: "NOMBRE 1" };
    const character2 = { id: "2", name: "NOMBRE 2" };
    it("Devuelve el los personajes existentes luego de ID inválido", async () => {
      const response = (await agent.delete("/rickandmorty/fav/1000")).body;
      expect(response).toContainEqual(character1);
      expect(response).toContainEqual(character2);
    });
